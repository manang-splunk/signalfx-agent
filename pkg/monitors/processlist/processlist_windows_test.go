...
func TestMapThreadToProcess(t *testing.T) {
	type args struct {
	...
	for i := range tests {
		tt := tests[i]
		t.Run(tt.name, func(t *testing.T) {
			if got := mapThreadToProcess(tt.args.threadList); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("mapThreadToProcess() = %v, want %v", got, tt.want)
			}
		})
	}
}
